...
software_name = 'Ink.Canvas.Setup'
software_name_download_first = 'WXRIW'
software_name_download_second = 'Ink-Canvas'
software_format = 'exe'
software_icon = './icon/Ink_Canvas_software.png'
software_info = '简介\n学校从传统投影仪换成了希沃白板，由于自带的“希沃白板”软件太难用，\n也没有同类好用的画板软件，所以开发了该画板。'
software_author = 'WXRIW'
...
class Ink_Canvas_Widget(QFrame):
    ...
    async def async_download(self, download_url, download_path, release_name): # type: ignore
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(download_url, timeout=timeout) as response: # type: ignore
                    ...
                    with open(os.path.join(download_path, f'{software_name}.Ver.{release_name}-release.{software_format}'), 'wb') as file: # type: ignore
                        while True:
    ...
    def download_latest_version(self):
        ...
        # 去除版本号中的 'v'
        if release_name.startswith('v'): # type: ignore
            release_name = release_name[1:] # type: ignore
        ...
        if isinstance(source_combo_box_data, dict):
            ...
            if source_combo_box_value == 'github':
                download_url = f"https://github.com/{software_name_download_first}/{software_name_download_second}/releases/download/{release_name}/{software_name}.Ver.{release_name}-release.{software_format}"
                ...
                download_url = f"https://ghfast.top/https://github.com/{software_name_download_first}/{software_name_download_second}/releases/download/{release_name}/{software_name}.Ver.{release_name}-release.{software_format}"
            else:
                download_url = f"https://github.com/{software_name_download_first}/{software_name_download_second}/releases/download/{release_name}/{software_name}.Ver.{release_name}-release.{software_format}"
        else:
            download_url = f"https://github.com/{software_name_download_first}/{software_name_download_second}/releases/download/{release_name}/{software_name}.Ver.{release_name}-release.{software_format}"
        ...
        if isinstance(download_combo_box_data, dict):
            download_combo_box_value = download_combo_box_data.get(f'{software_name}_download', 'None') # type: ignore
        ...
        if isinstance(download_manner_combo_box_data, dict):
            ...
            if download_manner_combo_box_value == '多线程(ThreadPoolExecutor)':
                ...
                timeout = 60
                with ThreadPoolExecutor() as executor:
                    future = executor.submit(urllib.request.urlretrieve, download_url, os.path.join(download_path, f'{software_name}.Ver.{release_name}-release.{software_format}'))
                    try:
                ...
                try:
                    ...
                    urllib.request.urlretrieve(download_url, os.path.join(download_path, f'{software_name}.Ver.{release_name}-release.{software_format}')) # type: ignore
                    logger.info(f"下载已完成: {download_url}")
                    self.showTeachingTip_download_latest_version_info_download()
                except TimeoutError:
                    logger.error(f"下载超时: {download_url}")
                ...
                try:
                    ...
                    with open(os.path.join(download_path, f'{software_name}.Ver.{release_name}-release.{software_format}'), 'wb') as file:
                ...
                # 使用 aiohttp 进行异步下载
                asyncio.run(self.async_download(download_url, download_path, release_name)) # type: ignore
            else:
