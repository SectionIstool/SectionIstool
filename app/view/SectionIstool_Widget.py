...
# from readme_content import show_readme_content_dialog # type: ignore
from ..common.config import load_custom_font

socket.setdefaulttimeout(None)

software_name_download_first = 'SectionIstool'
software_name_download_second = 'SectionIstool'
software_icon = './app/resource/icon/SectionIstool_icon.png'
software_info = '简介\nSectionIstool 是一款 班级一体机中方便、快捷的下载、管理多种软件'
software_author = 'lzy98276'
software_github = f'https://github.com/{software_name_download_first}/{software_name_download_second}'
software_bilibili = 'https://space.bilibili.com/520571577'
...
class sectionistool_Widget(QFrame):
    def __init__(self, parent: QFrame = None): # type: ignore
        ...
        # # 创建查看当前选择版本的更新日志按钮
        # self.readme_content_button = PushButton("查看当前选择版本的 README", inner_frame)
        # self.readme_content_button.setObjectName("downloadButton")
        # self.readme_content_button.setMinimumWidth(150)
        # # 设置按钮的提示信息
        # self.readme_content_button.setToolTip(f'点击即可查看当前选择版本的 README ✨')
        # self.readme_content_button.setToolTipDuration(1500)
        # self.readme_content_button.installEventFilter(ToolTipFilter(self.readme_content_button, showDelay=300, position=ToolTipPosition.TOP))
        # # 连接按钮的 clicked 信号到下载函数
        # self.readme_content_button.clicked.connect(lambda index: show_readme_content_dialog(software_name_download_second)) # type: ignore
        ...
        # # 创建刷新按钮
        # self.refresh_readme_content_button = PushButton("刷新 README", inner_frame)
        # self.refresh_readme_content_button.setObjectName("refreshButton")
        # self.refresh_readme_content_button.setMinimumWidth(150)
        # # 设置按钮的提示信息
        # self.refresh_readme_content_button.setToolTip('点击即可刷新 README 文件 刷新时间有点久哦~, 请耐心等待 ✨')
        # self.refresh_readme_content_button.setToolTipDuration(2500)
        # self.refresh_readme_content_button.installEventFilter(ToolTipFilter(self.refresh_readme_content_button, showDelay=300, position=ToolTipPosition.TOP))
        # # 连接按钮的 clicked 信号到刷新函数
        # self.refresh_readme_content_button.clicked.connect(self.showTeachingTip_refresh_download_readme_content) # type: ignore
        # self.refresh_readme_content_button.clicked.connect(refresh_download_readme_content) # type: ignore
    ...
    # def refresh_download_readme_content(self): # type: ignore
    #     # 读取配置文件中的仓库最新版本分支
    #     target_commitish_latest = read_json(f'./app/resource/releases/{software_name_download_second}_releases.json') # type: ignore
    #     # 提取 target_commitish 字段的值
    #     if isinstance(target_commitish_latest, list) and len(target_commitish_latest) > 0:  # type: ignore
    #         README_md_download_url = f'https://raw.githubusercontent.com/{software_name_download_first}/{software_name_download_second}/{target_commitish_latest[0]["target_commitish"]}/README.md'
    #     else:
    #         README_md_download_url = f'https://raw.githubusercontent.com/{software_name_download_first}/{software_name_download_second}/master/README.md'

    #     # 下载 README.md 文件
    #     with ThreadPoolExecutor() as executor:
    #         future = executor.submit(urllib.request.urlopen, README_md_download_url) # type: ignore
    #         response = future.result()
    #         if response.getcode() == 200:
    #             # 读取为字符串，而不是 JSON
    #             releases_data = response.read().decode('utf-8')
    #             # 确保 README 文件夹存在
    #             os.makedirs(f'README/{software_name_download_second}', exist_ok=True)
    #             with open(f'README/{software_name_download_second}/README.md', 'w', encoding='utf-8') as file:
    #                 file.write(releases_data)  # 保存为字符串
    #             logger.info(f"已成功下载 {software_name_download_second} 的 README.md 文件 ✨")
    #         else:
    #             logger.error(f"无法下载 {software_name_download_second} 的 README.md 文件")
    #             return
    ...
    # def showTeachingTip_refresh_download_readme_content(self):
    #     TeachingTip.create( # type: ignore
    #         target=self.refresh_readme_content_button,
    #         icon=InfoBarIcon.SUCCESS,
    #         title='刷新成功',
    #         content=f"已成功刷新 README 文件 ✨",
    #         isClosable=True,
    #         tailPosition=TeachingTipTailPosition.BOTTOM,
    #         duration=5000,
    #         parent=self
    #     )
    #     logger.info(f"已成功刷新 README 文件 ✨")
