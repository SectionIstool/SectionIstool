...
# 显示更新日志窗口的函数
def show_readme_dialog(self, readme_button, software_name_download_second) -> None: # type: ignore
    self.readme_button = readme_button
    ...
    try:
        logger.info("显示更新日志")
        readme_window = QWidget()  # 将 QFrame 更改为 QWidget
        ...
        text_edit = CustomTextBrowser(readme_window)  # 使用自定义的 QTextBrowser
        ...
        if download_version == 'Unknown':
            TeachingTip.create( # type: ignore
                target=self.readme_button, # type: ignore
                icon=InfoBarIcon.ERROR,
                title='获取版本号失败',
                content=f"无法获取 {software_name_download_second} 更新日志, 请 刷新软件版本, 详情请查看日志文件 ✨",
                isClosable=True,
                tailPosition=TeachingTipTailPosition.BOTTOM,
                duration=2000,
                parent=self # type: ignore
            )
            logger.error(f"下载失败, 无法读取 {software_name_download_second} 版本号, 详情请查看日志文件 ✨")
            return
        ...
        text_edit.setMarkdown(content) # type: ignore
