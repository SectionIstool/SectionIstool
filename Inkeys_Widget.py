...
software_name = 'Inkeys'
software_name_download_first = 'Alan-CRL'
software_name_download_second = 'Inkeys'
software_format = 'zip'
software_icon = './icon/Inkeys_software.png'
software_info = '简介\nWindows 屏幕批注工具,拥有高效批注和丰富功能，\n让屏幕演示变得简单,让教学授课变得高效,适用于触摸设备和PC端。'
software_author = 'Alan-CRL'
software_github = f'https://github.com/{software_name_download_first}/{software_name_download_second}'
software_bilibili = 'https://space.bilibili.com/1330313497'
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(download_url, timeout=timeout) as response: # type: ignore
                    ...
                    with open(os.path.join(download_path, f'{software_name}{release_name}-64.{software_format}'), 'wb') as file: # type: ignore
                        while True:
    ...
    def download_latest_version(self):
        ...
        if isinstance(source_combo_box_data, dict):
            ...
            if source_combo_box_value == 'github':
                download_url = f"https://github.com/{software_name_download_first}/{software_name_download_second}/releases/download/{release_name}/{software_name}{release_name}-64.{software_format}"
                ...
                download_url = f"https://ghfast.top/https://github.com/{software_name_download_first}/{software_name_download_second}/releases/download/{release_name}/{software_name}{release_name}-64.{software_format}"
            else:
                download_url = f"https://github.com/{software_name_download_first}/{software_name_download_second}/releases/download/{release_name}/{software_name}{release_name}-64.{software_format}"
        else:
            download_url = f"https://github.com/{software_name_download_first}/{software_name_download_second}/releases/download/{release_name}/{software_name}{release_name}-64.{software_format}"
        ...
        if isinstance(download_manner_combo_box_data, dict):
            ...
            if download_manner_combo_box_value == '多线程(ThreadPoolExecutor)':
                ...
                timeout = 60
                with ThreadPoolExecutor() as executor:
                    future = executor.submit(urllib.request.urlretrieve, download_url, os.path.join(download_path, f'{software_name}{release_name}-64.{software_format}'))
                    try:
                ...
                try:
                    ...
                    urllib.request.urlretrieve(download_url, os.path.join(download_path, f'{software_name}{release_name}-64.{software_format}')) # type: ignore
                    logger.info(f"下载已完成: {download_url}")
                    self.showTeachingTip_download_latest_version_info_download()
                except TimeoutError:
                    logger.error(f"下载超时: {download_url}")
                ...
                try:
                    ...
                    with open(os.path.join(download_path, f'{software_name}{release_name}-64.{software_format}'), 'wb') as file:
