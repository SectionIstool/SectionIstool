        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(download_url, timeout=timeout) as response: # type: ignore
                    ...
                    with open(os.path.join(download_path, f'{software_name}.{release_name}.{software_format}'), 'wb') as file: # type: ignore
                        while True:
    ...
    def download_latest_version(self):
        ...
        if isinstance(source_combo_box_data, dict):
            ...
            if source_combo_box_value == 'github':
                download_url = f"https://github.com/{software_name_download_first}/{software_name_download_second}/releases/download/{release_name}/{software_name}.{release_name}.{software_format}"
                ...
                download_url = f"https://ghfast.top/https://github.com/{software_name_download_first}/{software_name_download_second}/releases/download/{release_name}/{software_name}.{release_name}.{software_format}"
            else:
                download_url = f"https://github.com/{software_name_download_first}/{software_name_download_second}/releases/download/{release_name}/{software_name}.{release_name}.{software_format}"
        else:
            download_url = f"https://github.com/{software_name_download_first}/{software_name_download_second}/releases/download/{release_name}/{software_name}.{release_name}.{software_format}"
        ...
        if isinstance(download_manner_combo_box_data, dict):
            ...
            if download_manner_combo_box_value == '多线程(ThreadPoolExecutor)':
                ...
                timeout = 60
                with ThreadPoolExecutor() as executor:
                    future = executor.submit(urllib.request.urlretrieve, download_url, os.path.join(download_path, f'{software_name}.{release_name}.{software_format}'))
                    try:
                ...
                try:
                    ...
                    urllib.request.urlretrieve(download_url, os.path.join(download_path, f'{software_name}.{release_name}.{software_format}')) # type: ignore
                    logger.info(f"下载已完成: {download_url}")
                    self.showTeachingTip_download_latest_version_info_download()
                except TimeoutError:
                    logger.error(f"下载超时: {download_url}")
                ...
                try:
                    ...
                    with open(os.path.join(download_path, f'{software_name}.{release_name}.{software_format}'), 'wb') as file:
